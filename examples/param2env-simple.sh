...
# try also setting PEGASO_BASH_IMPORT_VERBOSE=0 ; see below.
...
declare -A PEGASO_VALID_ENV_PARAMS=(   [MODE]=upstring [PATCH]=bool   [PREPARE]=bool  [VER]=upstring )
declare -A PEGASO_VALID_ENV_DEFAULT=(  [MODE]=BB       [PATCH]=TRUE   [PREPARE]=FALSE [VER]=undef )
...
PEGASO_BASH_IMPORT_VERBOSE=1 # default is 0, disabled; othewise is the file descriptior to write to (1:stdout)
source $PEGASO_BASH_ROOT/pegaso-bash.sh param2env log || exit 3
...
if [ $# == 0 ]; then
    ...
    for unprocessed in "${PEGASO_ENV_PARAMS_NOT_PROCESSED[@]}"; do
